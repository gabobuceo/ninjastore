...
*/
	...
	class PersistenciaFactura{
		...
		public function consPendientes($conex){
			$sql = "SELECT FACTURA.*,PUBLICACION.TITULO FROM FACTURA, PUBLICACION WHERE ESTADO='PENDIENTE' AND FECHAV >= NOW() AND FACTURA.IDPUBLICACION = PUBLICACION.ID GROUP BY FACTURA.ID";
			$result = $conex->prepare($sql);
			$result->execute();
			$resultados=$result->fetchAll();
			return $resultados;
		}
		public function consVencidas($conex){
			$sql = "SELECT FACTURA.*,PUBLICACION.TITULO FROM FACTURA, PUBLICACION WHERE ( ESTADO='VENCIDA' OR FECHAV < NOW() ) AND FACTURA.IDPUBLICACION = PUBLICACION.ID GROUP BY FACTURA.ID";
			$result = $conex->prepare($sql);
			$result->execute();
			$resultados=$result->fetchAll();
			return $resultados;
		}
		public function consPagas($conex){
			$sql = "SELECT FACTURA.*,PUBLICACION.TITULO FROM FACTURA, PUBLICACION WHERE ESTADO='PAGA' AND FACTURA.IDPUBLICACION = PUBLICACION.ID GROUP BY FACTURA.ID";
			$result = $conex->prepare($sql);
			$result->execute();
			$resultados=$result->fetchAll();
			return $resultados;
		}
		public function pagFactura($obj, $conex){
			$id=trim($obj->getId());
			$sql = "UPDATE FACTURA SET ESTADO='PAGA' WHERE ID=:ID";
			$result = $conex->prepare($sql);
			$result->execute(array(":ID" => $id));
			if($result){
				return(true);
		...
		public function morar($obj, $conex){
			$id=trim($obj->getId());
			$vence=trim($obj->getFechav());
			$sql = "UPDATE FACTURA SET FECHAV=:FECHAV, ESTADO='PENDIENTE' WHERE ID=:ID";
			$result = $conex->prepare($sql);
			$result->execute(array(":ID" => $id,
				":FECHAV" => $vence));
			if($result){
